...
from rsl_rl.datasets import motion_util, pose3d
...
class AMPLoader:
    ...
    def get_full_frame_at_time_batch(self, traj_idxs, times):
        ...
        all_frame_amp_starts = torch.zeros(
            len(traj_idxs),
            AMPLoader.JOINT_VEL_END_IDX - AMPLoader.JOINT_POSE_START_IDX,
            device=self.device,
        )
    ...
    def blend_frame_pose(self, frame0, frame1, blend):
        ...
        root_pos0, root_pos1 = AMPLoader.get_root_pos(frame0), AMPLoader.get_root_pos(
            frame1
        )
        root_rot0, root_rot1 = AMPLoader.get_root_rot(frame0), AMPLoader.get_root_rot(
            frame1
        )
        joints0, joints1 = AMPLoader.get_joint_pose(frame0), AMPLoader.get_joint_pose(
            frame1
        )
        tar_toe_pos_0, tar_toe_pos_1 = AMPLoader.get_tar_toe_pos_local(
            frame0
        ), AMPLoader.get_tar_toe_pos_local(frame1)
        linear_vel_0, linear_vel_1 = AMPLoader.get_linear_vel(
            frame0
        ), AMPLoader.get_linear_vel(frame1)
        angular_vel_0, angular_vel_1 = AMPLoader.get_angular_vel(
            frame0
        ), AMPLoader.get_angular_vel(frame1)
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        ...
        for _ in range(num_mini_batch):
            if self.preload_transitions:
                ...
                s = self.preloaded_s[
                    idxs, AMPLoader.JOINT_POSE_START_IDX : AMPLoader.JOINT_VEL_END_IDX
                ]
                s = torch.cat(
                    [
                        s,
                        self.preloaded_s[
                            idxs,
                            AMPLoader.ROOT_POS_START_IDX
                            + 2 : AMPLoader.ROOT_POS_START_IDX
                            + 3,
                        ],
                    ],
                    dim=-1,
